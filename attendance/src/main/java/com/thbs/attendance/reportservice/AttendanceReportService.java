...
public class AttendanceReportService {
	...
	public List<ReportExcelDto> generateAttendanceReportDetails(Long batchId, Long courseId, String startDate, String endDate) {
	    ...
	    // Ensure batch exists in attendance records
	    if (attendances.isEmpty()) {
	        throw new BatchNotFoundException("No attendance records found for batch id " + batchId);
	    }

	    // Ensure course exists in attendance records for the given batch and dates
	    boolean courseFound = attendances.stream().anyMatch(attendance ->
	            attendance.getCourseId().equals(courseId));
	    if (!courseFound) {
	        throw new CourseNotFoundException("No attendance records found for course id " + courseId +
	                " within the specified date range for batch id " + batchId);
	    }

	    // Generate attendance report DTOs
	    List<ReportExcelDto> attendanceReport = new ArrayList<>();
	    for (Long employeeId : employeeIds) {
	        Map<String, Object> employeeDetail = employeeDetailsMap.get(employeeId);
	        ...
	        reportDto.setBatchId(batchId);
	        reportDto.setCourseId(courseId);
	        reportDto.setUserId(employeeId);
	        ...
	        while (!currentDate.isAfter(endDateParsed)) {
	            ...
	            List<AttendanceDetail> detailsForDate = attendances.stream()
	                    .filter(a -> a.getUserId().equals(employeeId))
	                    .flatMap(a -> a.getAttendance().stream())
	                    .filter(detail -> detail.getDate().equals(formattedDate))
	...
	public List<ReportExcelDto> generateAttendanceReportDetails(Long batchId, Long courseId, String date) {
		...
		if (attendances.isEmpty()) {
			throw new CourseNotFoundException("No attendance records found on " +  date + " for courseId " + courseId);
		}
	...
	public void generateAttendanceExcelFile(Long batchId, Long courseId, String date) throws IOException {
		try {
			...
			// Handle course not found exception
			throw new CourseNotFoundException("No attendance records found on " +  date  + " for course id " + courseId);
		}
	}	
	
	
	//ATTENDANCE LIST OF EMPLOYEE OF A BATCH ALONG WITH COURSEID ,EMPLOYEEID AND  DATE RANGE  IN JSON FORMAT
	...
	//ATTENDANCE LIST OF EMPLOYEE OF A BATCH ALONG WITH COURSEID ,EMPLOYEEID AND  DATE RANGE  IN EXCEL FILE
	...
	//ATTENDANCE LIST OF EMPLOYEE OF A BATCH ALONG WITH COURSEID ,EMPLOYEEID AND  PARTICULAR DATE  IN JSON FORMAT
	public List<ReportExcelDto> generateEmployeeAttendanceReport(Long batchId, Long courseId, Long employeeId,
			String date) {
		try {
			String BATCH_MANAGEMENT_SERVICE_URL = "http://BATCH-MANAGEMENT-SERVICE/batch/batch-details/employees/{batchId}";
			...
			if (attendances.isEmpty()) {
	            throw new CourseNotFoundException("Course with ID " + courseId + " not found for the specified date, employee and batch");
	        }
	...
	//ATTENDANCE LIST OF EMPLOYEE OF A BATCH ALONG WITH COURSEID ,EMPLOYEEID AND PARTICULAR  DATE IN EXCEL FORMAT
